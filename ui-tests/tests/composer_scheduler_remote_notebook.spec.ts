...
test.describe('Composer scheduling jobs for cluster', () => {
  ...
  test('Can create a job scheduler with Run on a schedule', async ({
    ...
    test.setTimeout(timeout);
    await createJobSchedulerCluster(page, 'Run on a schedule');
  });
  ...
  test('CMP-02,CMP-03,CMP-04,CMP-05,CMP-06,CMP-07,CMP-08,CMP-09,CMP-10,CMP-11,CMP-12,CMP-13,CMP-14,CMP-15,CMP-20,CMP-17,CMP-18,CMP-19,CMP-22,CMP-23,CMP-24,CMP-25,CMP-26,CMP-27.CMP-28,CMP-29,CMP-30,VTX-02:Sanity: can perform field validation', async ({
    page
  }) => {
    test.setTimeout(150 * 1000);
    ...
    if (kernelCount > 0) {
      ...
      await page.getByText('Notebook', { exact: true }).nth(1).isVisible();
      ...
      // await expect(page.getByLabel('Cluster')).toBeChecked();
      await expect(
        page.getByRole('radiogroup').nth(1).getByTestId('cluster-selected')
      ).toBeTruthy();
      await expect(page.getByLabel('Serverless')).not.toBeChecked();
      ...
      await page.getByLabel('Retry delay (minutes)').clear();
      await page.getByLabel('Retry delay (minutes)').fill('2');
...
test.describe('CMP-21,CMP-31:Composer scheduling jobs for serverless', () => {
  ...
  test('Can create a job scheduler with Run on a schedule', async ({
    ...
    test.setTimeout(timeout);
    await createJobSchedulerServerless(page, 'Run on a schedule');
  });
  ...
  test('CMP-31:Cancel job creation', async ({ page }) => {
    ...
    const locator = page.locator('.jp-LauncherCard:visible', {
      hasText: 'on Serverless Spark (Remote)'
    });
    await page.waitForTimeout(20000);
    const count = await locator.count();
    expect(count).toBeGreaterThan(0);
    if (count > 0) {
      ...
      // Select the first available option for dropdown fields
      const dropdownFields = ['Environment*', 'Serverless*'];
  ...
  test('CMP-02,CMP-03,CMP-04,CMP-05,CMP-06,CMP-07,CMP-08,CMP-09,CMP-10,CMP-11,CMP-12,CMP-13,CMP-14,CMP-15,CMP-16,CMP-18,CMP-19,CMP-22,CMP-23,CMP-24,CMP-25,CMP-26,CMP-27.CMP-28,CMP-29,CMP-30:Sanity: can perform field validation', async ({
    page
  }) => {
    test.setTimeout(150 * 1000);
    ...
    if (kernelCount > 0) {
      ...
      // await expect(page.getByLabel('Serverless').nth(1)).toBeChecked();
      // await expect(
      //   page.locator("//input[@type='radio' and @value='serverless']")
      // ).toBeVisible();
      await expect(
        page.getByRole('radiogroup').getByTestId('serverless-selected')
      ).toBeTruthy();
      await expect(page.getByLabel('Cluster')).not.toBeChecked();
...
test.describe('Composer scheduling jobs listing page validation', () => {
  ...
  test('CMP-46:Edit a schedule', async ({ page }) => {
    ...
    if (jobLocator) {
      ...
      await expect(
        page.locator("//div[contains(@class,'disable-item')]")
      ).toBeVisible();
